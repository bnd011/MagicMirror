...
class GUI(Frame):
    ...
    def setupGUI(self):
        ...
        GUI.textTime = Text(time_frame, bg='black', fg='white', state=DISABLED, font=self.largeFont, bd=0,
                            highlightbackground='black')  # Creates what is essentially an empty text box
    ...
    def dispWeather(self, weather, frameNum):
        ...
        if (frameNum == 0):
            ...
            GUI.textW0.insert(END, "  {} {} | High:{} {} | Low: {} {}".format(weather.currentTemp, deg, weather.high, deg,
                                                                            weather.low, deg))

        if (frameNum == 1):
            GUI.textW1.delete("1.0", END)
            GUI.textW1.config(state=NORMAL)
            GUI.textW1.insert(END,
                              "{}:   High:{} {} | Low: {} {}".format(weather.date, weather.high, deg, weather.low, deg))
            GUI.textW1.image_create(END, image=img1)

        if (frameNum == 2):
            GUI.textW2.delete("1.0", END)
            GUI.textW2.config(state=NORMAL)
            GUI.textW2.insert(END,
                              "{}:   High:{} {} | Low: {} {}".format(weather.date, weather.high, deg, weather.low, deg))
    ...
    def updateGUI(self):
        ...
        for event in events:
            ...
            self.dispEvent(start, event['summary'])
        ########################
...
# Sample events
event1 = setEvent("05/16/2018", "10:30", "CYEN Presentation")
event2 = setEvent("05/18/2018", "8:00", "Physics Test 3")
event3 = setEvent("05/18/2018", "13:30", "CYEN Test 3")
